...
@media (max-width: 620px) {
    ...
    /* Columns stack vertically on mobile (natural Streamlit behavior).
       The left column already contains [headline, subhead, MOBILE PREVIEW,
       form, proof] in that DOM order — exactly what we want. The right
       column contains only the desktop preview which we hide above. */
...
/* Mobile-only preview iframe — hidden on desktop, shown on mobile */
.msp-preview-mobile {
    display: none;
}
.msp-preview-desktop {
...
""",
    ...
    # Hidden on desktop via CSS; visible only at <= 620px.
    st.markdown('<div class="msp-preview-mobile">', unsafe_allow_html=True)
...
with right:
    st.markdown('<div class="msp-preview-desktop">', unsafe_allow_html=True)
