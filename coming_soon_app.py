...
def load_signups():
    if os.path.exists(SIGNUPS_FILE):
        try:
            return pd.read_csv(SIGNUPS_FILE)
        except Exception:
            return pd.DataFrame(columns=["timestamp", "name", "email", "source"])
    return pd.DataFrame(columns=["timestamp", "name", "email", "source"])

def save_signup(name, email, source="coming_soon"):
    df = load_signups()
    normalized_email = email.strip().lower()

    if not df.empty and normalized_email in df["email"].astype(str).str.lower().values:
...
def send_signup_email(row):
    if not RESEND_API_KEY:
        return False, "RESEND_API_KEY is not set. Signup was saved locally."
    ...
    <div style="font-family:Inter,Arial,sans-serif;background:#07090f;color:#e2e8f0;padding:32px;border-radius:14px;">
      <h2 style="margin:0 0 14px;color:#ffffff;">
        Market<span style="color:#f59e0b;">Signal</span>Pro
      </h2>
      <h3 style="margin:0 0 20px;color:#60a5fa;">New early-access signup</h3>
      <p><strong>Name:</strong> {row["name"]}</p>
      <p><strong>Email:</strong> {row["email"]}</p>
      <p><strong>Time:</strong> {row["timestamp"]}</p>
      <p style="color:#7c8fae;font-size:12px;margin-top:24px;">
        This was submitted from the MarketSignalPro coming-soon page.
      </p>
    </div>
    """
...
.msp-launch-card {
    ...
    padding: 20px;
    border: 1px solid var(--line);
    background: linear-gradient(180deg, rgba(13,21,37,.78), rgba(8,12,22,.65));
    border-radius: 22px;
...
.msp-dashboard {
    ...
    background:
        linear-gradient(180deg, rgba(13, 21, 37, .92), rgba(6, 10, 19, .88)),
        radial-gradient(circle at 80% 10%, rgba(37,99,235,.2), transparent 35%);
    border-radius: 28px;
    box-shadow: 0 40px 120px rgba(0,0,0,.65), 0 0 80px rgba(37,99,235,.16);
    overflow: hidden;
...
.msp-traffic i:nth-child(1) { background: #ef4444; }
.msp-traffic i:nth-child(2) { background: #f59e0b; }
.msp-traffic i:nth-child(3) { background: #22c55e; }
...
.msp-score {
    ...
    place-items: center;
    border-radius: 24px;
    background: linear-gradient(135deg, rgba(34,197,94,.18), rgba(37,99,235,.16));
...
.msp-ai-card {
    ...
    overflow: hidden;
    background:
        radial-gradient(circle at 30% 25%, rgba(96,165,250,.55), transparent 25%),
        radial-gradient(circle at 72% 68%, rgba(245,158,11,.32), transparent 24%),
        linear-gradient(135deg, #07111f, #030712);
}

.msp-ai-card:nth-child(2n) {
    background:
        radial-gradient(circle at 70% 22%, rgba(34,197,94,.32), transparent 24%),
        radial-gradient(circle at 20% 78%, rgba(37,99,235,.5), transparent 26%),
        linear-gradient(135deg, #070b15, #100a1d);
}

.msp-ai-card:nth-child(3n) {
    background:
        radial-gradient(circle at 50% 30%, rgba(245,158,11,.36), transparent 24%),
        radial-gradient(circle at 78% 70%, rgba(96,165,250,.42), transparent 22%),
        linear-gradient(135deg, #06101d, #090914);
}
...
.msp-ai-card::before {
    ...
    inset: 0;
    background:
        linear-gradient(115deg, transparent 20%, rgba(255,255,255,.12), transparent 45%),
...
<div class="msp-page">
  ...
  <main class="msp-hero">
    <section>
      ...
      <div class="msp-launch-card" id="early-access">
        <div class="msp-launch-card-title">
...
with st.container():
    # This HTML spacing keeps the Streamlit form visually inside the launch card.
    st.markdown('<div style="width:min(640px, calc(100vw - 44px)); margin: -470px auto 0; opacity:0; height:0; pointer-events:none;"></div>', unsafe_allow_html=True)

# Use columns to align the actual form over the card area only on normal Streamlit flow.
# The form is placed immediately after the card header in DOM flow by closing/opening HTML around it.
with st.form("early_access_form", clear_on_submit=True):
    col1, col2, col3 = st.columns([1.0, 1.15, 0.82], gap="small")
    with col1:
    ...
    if submitted:
        ...
        if not name_clean or not email_clean:
            ...
            ok, result = save_signup(name_clean, email_clean)
...
st.markdown(
    ...
    <section class="msp-visual" aria-label="MarketSignalPro product preview">
      <div class="msp-orbit"></div>
      <div class="msp-orbit two"></div>

      <div class="msp-dashboard">
        <div class="msp-dash-top">
          <div class="msp-traffic"><i></i><i></i><i></i></div>
          <div class="msp-dash-label">MARKETSIGNALPRO / LIVE PREVIEW</div>
        </div>
        <div class="msp-dash-body">
          <div class="msp-signal-title">
            <h2>Signal Matrix<br><span>Opportunity Radar</span></h2>
            <div class="msp-score">92</div>
          </div>

          <div class="msp-stock-row">
            <div class="msp-ticker">NVDA</div>
            <div class="msp-bar"><span style="width:92%"></span></div>
            <div class="msp-row-tag">Strong Buy</div>
          </div>
          <div class="msp-stock-row">
            <div class="msp-ticker">TSLA</div>
            <div class="msp-bar"><span style="width:78%"></span></div>
            <div class="msp-row-tag">Momentum</div>
          </div>
          <div class="msp-stock-row">
            <div class="msp-ticker">AMD</div>
            <div class="msp-bar"><span style="width:68%"></span></div>
            <div class="msp-row-tag">Watch</div>
          </div>

          <div class="msp-insight-box">
            <strong>Plain-English insight</strong>
            <p>
              Momentum, trend, volume, and sentiment are aligning. MarketSignalPro translates
              the signal stack into simple, actionable context.
            </p>
          </div>
        </div>
      </div>
  ...
  <div class="msp-ticker-tape" aria-hidden="true">
    <div class="msp-ticker-track">
      <div class="msp-ticker-chip"><b>AI Signals</b><span>+ Early</span></div>
      <div class="msp-ticker-chip"><b>Momentum</b><span>+ Radar</span></div>
      <div class="msp-ticker-chip"><b>Sentiment</b><span>+ Shift</span></div>
      <div class="msp-ticker-chip"><b>Squeeze</b><span>+ Watch</span></div>
      <div class="msp-ticker-chip"><b>Volume</b><span>+ Surge</span></div>
      <div class="msp-ticker-chip"><b>Market</b><span>+ Scan</span></div>
      <div class="msp-ticker-chip"><b>AI Signals</b><span>+ Early</span></div>
      <div class="msp-ticker-chip"><b>Momentum</b><span>+ Radar</span></div>
      <div class="msp-ticker-chip"><b>Sentiment</b><span>+ Shift</span></div>
      <div class="msp-ticker-chip"><b>Squeeze</b><span>+ Watch</span></div>
  ...
  <section class="msp-section">
    ...
    <div class="msp-feature-grid">
      <article class="msp-feature">
        ...
        <h4>AI-focused analysis</h4>
        <p>Signals are organized into plain-English context so users do not have to decode every metric manually.</p>
      </article>
      <article class="msp-feature">
        <div class="msp-feature-icon">📡</div>
        <h4>Momentum and sentiment</h4>
        <p>Track attention, volume, and market movement in one place instead of jumping between tools.</p>
      </article>
      <article class="msp-feature">
        <div class="msp-feature-icon">⚡</div>
        <h4>Built for speed</h4>
        <p>Designed around quick discovery, cleaner watchlists, and faster decisions when markets are moving.</p>
      </article>
    </div>

    <div class="msp-gallery-shell">
      <div class="msp-gallery-track">
        <div class="msp-ai-card" data-label="AI Signal Engine"></div>
        <div class="msp-ai-card" data-label="Opportunity Radar"></div>
        <div class="msp-ai-card" data-label="Market Sentiment"></div>
        <div class="msp-ai-card" data-label="Momentum Scanner"></div>
        <div class="msp-ai-card" data-label="Smart Watchlists"></div>
        <div class="msp-ai-card" data-label="Signal Scoring"></div>
        <div class="msp-ai-card" data-label="AI Signal Engine"></div>
        <div class="msp-ai-card" data-label="Opportunity Radar"></div>
        <div class="msp-ai-card" data-label="Market Sentiment"></div>
        <div class="msp-ai-card" data-label="Momentum Scanner"></div>
...
# Optional hidden admin view:
# Add ?admin=1 to the URL to download current signups.
try:
