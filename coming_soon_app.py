...
@media (max-width: 620px) {
    ...
    /* ── Mobile hero order: headline → preview → form → proof ─

       Streamlit DOM looks like:
         row
           col-left
             vblock
               markdown (headline+subhead)   ← want order 1
               form                          ← want order 3
               markdown (proof pills)        ← want order 4
           col-right
             vblock
               iframe (live preview)         ← want order 2

       Strategy: turn the row into a flex column, then use
       display:contents to "flatten" the columns and inner vblocks
       so the 4 leaf items (headline-markdown, form, proof-markdown,
       iframe) become direct flex children of the row. Then order
       them with the `order` CSS property.
    */
    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow),
    div[data-testid="stHorizontalBlock"]:has(.msp-eyebrow) {
    ...
    /* Flatten BOTH columns + their inner vertical blocks so leaf
       items rise to be flex children of the hero row above. */
    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow) > [data-testid="column"],
    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow) > [data-testid="column"] > [data-testid="stVerticalBlock"],
    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow) > [data-testid="column"] > div > [data-testid="stVerticalBlock"] {
        display: contents !important;
    }

    /* Now order the surfaced leaf elements:
       1 — headline + subhead markdown block
       2 — iframe (the live preview, only direct child of right column)
       3 — form
       4 — proof pills markdown block
    */
    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow) [data-testid="stMarkdownContainer"]:has(.msp-h1) {
        order: 1 !important;
        width: 100% !important;
    }
    ...
    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow) [data-testid="column"]:last-of-type [data-testid="element-container"] {
        order: 2 !important;
        width: 100% !important;
    }

    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow) [data-testid="stForm"] {
        order: 3 !important;
        width: 100% !important;
    }

    [data-testid="stHorizontalBlock"]:has(.msp-eyebrow) [data-testid="stMarkdownContainer"]:has(.msp-proof) {
        order: 4 !important;
        width: 100% !important;
    }
