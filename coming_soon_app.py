...
@media (max-width: 620px) {
    ...
    /* ── Mobile preview iframe: show inline preview, hide desktop preview ── */
    /* Show the iframe that follows the mobile marker */
    [data-testid="element-container"]:has(.msp-iframe-mobile-marker)
        + [data-testid="element-container"] {
        display: block !important;
        width: 100% !important;
        margin: 8px auto 4px !important;
    }
    /* Hide the iframe that follows the desktop marker */
    [data-testid="element-container"]:has(.msp-iframe-desktop-marker)
        + [data-testid="element-container"] {
        display: none !important;
    }

    /* Collapse the right column on mobile — its only content (desktop
       preview) is hidden, so the column shouldn't take any space. */
    ...
    /* ── Form card — smaller, centered, room from edges ── */
    [data-testid="stForm"] {
...
/* ============================================================
   IFRAME VISIBILITY TOGGLE — desktop vs mobile preview iframes.

   Streamlit's `st.markdown('<div class="X">', ...)` does NOT wrap
   subsequent `components.html()` calls — they become siblings, not
   parent/child. So we use empty marker divs placed BEFORE each
   iframe, then target the iframe's actual container via:

     [data-testid="element-container"]:has(.marker)
       + [data-testid="element-container"]

   `:has()` finds the marker's container, `+` jumps to the next
   sibling (which holds the iframe).
   ============================================================ */

/* The marker divs themselves are visually invisible */
.msp-iframe-mobile-marker,
.msp-iframe-desktop-marker {
    display: none;
}

/* DESKTOP DEFAULT: hide the mobile iframe (the one after the mobile marker) */
[data-testid="element-container"]:has(.msp-iframe-mobile-marker)
    + [data-testid="element-container"] {
    display: none !important;
}
/* Also hide the marker's own container so it doesn't take space */
[data-testid="element-container"]:has(.msp-iframe-mobile-marker) {
    display: none !important;
}
[data-testid="element-container"]:has(.msp-iframe-desktop-marker) {
...
""",
    ...
    # Mobile-only live-preview iframe (between headline+subhead and the form).
    # A marker div is placed BEFORE the iframe; CSS uses
    # `:has(.msp-iframe-mobile-marker) + [data-testid="element-container"]`
    # to target the iframe's actual container and toggle its visibility.
    st.markdown('<div class="msp-iframe-mobile-marker"></div>', unsafe_allow_html=True)
...
with right:
    st.markdown('<div class="msp-iframe-desktop-marker"></div>', unsafe_allow_html=True)
