...
/* ── Streamlit wrapper hardening ─────────────────────────────
   Streamlit wraps every heading in a generated container (class names
   like st-emotion-cache-… / stHeading and a data-testid) and appends an
   anchor-link element. Across Streamlit versions that wrapper can be
   shrink-to-content or the anchor can sit inline, which throws the
   heading off the column center. These rules force the wrapper to span
   the full step width and center its text no matter the version, and
   hide the injected anchor so it can't add asymmetric width. */
.msp-story-step [data-testid="stHeading"],
.msp-story-step [data-testid="stHeadingWithActionElements"],
.msp-story-step [class*="stHeading"],
.msp-story-step h4 {
    width: 100% !important;
    max-width: 100% !important;
...
.msp-story-step h4 {
    align-self: stretch !important;   /* fill the flex column, then center text */
}
.msp-story-step h4 a,
.msp-story-step [data-testid="stHeaderActionElements"],
.msp-story-step [data-testid="StyledLinkIconContainer"] {
    display: none !important;
}
...
/* Constrain the hero block specifically.
   NOTE: the eyebrow is rendered ABOVE the columns, so :has(.msp-eyebrow)
   never matched the hero block and it stretched full-width — pushing the
   form and preview to opposite edges. The form lives INSIDE this block,
   so :has(.msp-launch-title) reliably targets it. It's the only
   st.columns block on the page, so this can't collide with anything. */
section.main [data-testid="stHorizontalBlock"]:has(.msp-launch-title),
section.main div.stHorizontalBlock:has(.msp-launch-title),
[data-testid="stHorizontalBlock"]:has(.msp-launch-title),
div.stHorizontalBlock:has(.msp-launch-title) {
...
/* ============================================================
   HERO ENTRANCE ANIMATIONS — draw the eye to the form & preview
   Pop-in on load (fade + rise + slight scale), staggered, plus a
   brief glow pulse on the form to pull attention after it lands.
...
@keyframes mspPopInRight {
    0%   { opacity: 0; transform: translateX(34px) scale(.97); }
    100% { opacity: 1; transform: translateX(0) scale(1); }
}
@keyframes mspAttnGlow {
    0%, 100% { box-shadow: 0 0 0 0 rgba(96,165,250,0),
                           0 18px 60px rgba(0,0,0,.45); }
    50%      { box-shadow: 0 0 0 3px rgba(96,165,250,.30),
                           0 0 46px rgba(96,165,250,.34),
                           0 18px 60px rgba(0,0,0,.45); }
}

/* Form: rise + fade in, then a couple of attention glow pulses */
[data-testid="stForm"]:has(.msp-launch-title) {
    animation:
        mspPopIn .72s cubic-bezier(.2,.85,.25,1) .12s both,
        mspAttnGlow 2.1s ease-in-out 1.0s 2;
    will-change: transform, opacity, box-shadow;
}

/* Live preview iframe: slide in from the right, slightly later.
   The preview is the ONLY iframe nested inside a stColumn (the story
   gallery iframe sits directly under the main block container), so this
   selector animates the preview without touching that other section. */
[data-testid="stColumn"] iframe,
[data-testid="stColumn"] [data-testid="stElementContainer"]:has(iframe) {
    animation: mspPopInRight .8s cubic-bezier(.2,.85,.25,1) .34s both;
    will-change: transform, opacity;
}

/* Headline + subhead: gentle staggered rise so the whole hero feels alive */
.msp-h1     { animation: mspPopIn .6s cubic-bezier(.2,.85,.25,1) 0s   both; }
.msp-subhead{ animation: mspPopIn .6s cubic-bezier(.2,.85,.25,1) .06s both; }

@media (prefers-reduced-motion: reduce) {
    [data-testid="stForm"]:has(.msp-launch-title),
    section.main iframe[title="streamlit_app"],
    section.main [data-testid="stIFrame"]:has(iframe),
    .msp-h1, .msp-subhead {
